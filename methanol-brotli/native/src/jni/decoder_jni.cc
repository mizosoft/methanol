...
Java_com_github_mizosoft_methanol_brotli_internal_vendor_DecoderJNI_nativeCreate(
  ...
  jint input_size = static_cast<jint>(context[1]);
  if (static_cast<jlong>(input_size) != context[1] || input_size < 0) {
    ok = false; // Overflow or negative size.
  }
  context[0] = 0;
  context[2] = 0;
...
Java_com_github_mizosoft_methanol_brotli_internal_vendor_DecoderJNI_nativePull(
  ...
  env->SetLongArrayRegion(ctx, 0, 3, context);
  return env->NewDirectByteBuffer(const_cast<uint8_t*>(data), static_cast<jint>(data_length));
}
